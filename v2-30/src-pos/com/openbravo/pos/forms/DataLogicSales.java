    ...
    public final void saveTicket(final TicketInfo ticket, final String location) throws BasicException {
        ...
        Transaction t = new Transaction(s) {
            public Object transact() throws BasicException {
                ...
                // Set Receipt Id, si tiene  significa que no se le ha puesto ya el ID, se pone uno de acuerdo al tipo de Ticket ,,,
                ...
                //Parte de la insercion del NCF

                if(ticket.isNCF()==true){
                    DataLogicNCF dlNCF = new DataLogicNCF();
                    dlNCF.init(s);

                    EmpresaNCF e = dlNCF.obtenerEmpresaNCF();

                    //TipoNCF t = dlNCF.obtenerTipoNCF(1); // TiposNCF.Id = 1 es "VALIDO PARA CREDITO FISCAL"

                    RangoNCF r = dlNCF.obtenerRangoNCFPorTipoActivo(1); //Obtener el rango "ACTIVO" de este tipo

                    AsociacionNCF a = new AsociacionNCF();
                    a.init(ticket.getId(), r, ticket.getCustomer().getTaxid());
                    a.crearNCF(e);

                    dlNCF.setearAsociacionNCF(a);
                    ticket.setNCF(a.getOriginalNCF());

                    //Hemos utilizado un espacio del rango activo (que debe de tener disponible para serlo)
                    r.setContador(r.getContador()+1); //increntamos par areflejar uso

                    if(r.verifDisponible() == true){
                        // Si queda disponible solo hay que actualizar, para reflejar que se ha incrementado
                        dlNCF.actualizarRangoNCF(r);
                    }else{
                        //Si no quedan disponible debe de ponerse AGOTADO
                        EstadoRangoNCF nEst = new EstadoRangoNCF();
                        nEst.setEstado(EstadoRangoNCF.estadoAgotado);
                        r.setEstado(nEst); //Hya otra manera?, simplemente llamar el objeto original ??
                        
                        //En esta sobreescribimos el estado
                        RangoNCF r2 = dlNCF.obtenerRangoNCFPorTipoProximoInactivo(1);
                        dlNCF.actualizarRangoNCF(r);
                        if(r2 != null)
                        {
                            EstadoRangoNCF nEst2 = new EstadoRangoNCF();
                            nEst2.setEstado(EstadoRangoNCF.estadoActivo);
                            r2.setEstado(nEst2);
                            dlNCF.actualizarRangoNCF(r2);
                        }
                    }
