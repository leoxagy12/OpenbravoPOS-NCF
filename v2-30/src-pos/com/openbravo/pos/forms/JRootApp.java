 ...
 *
 * @author adrianromero
 * Esta clase muestra el panel del login y prepara la aparicion de JPrincipalApp m_principalapp
 * que se abre de acuerdo al usuario que se logea, tambien inicializa cosas com el
 * printer, sacale, llama a DataLogicSystem para inicializar la secuecia de efectivo etc
 */
public class JRootApp extends JPanel implements AppView {
 
    private AppProperties m_props;

    //Este objeto maneja la conexion a la base de datos ...
    private Session session;

    //Este objeto maneja la Logica de la data de la apliacion se le pasa variable
    //session en el metodo init(session) con el que se conecta a la base de datos
    //y extrae los datos basicos del sistema, activa la secuencia de dinero
    //tiene la lista de los usuarios que se pueden logear, etc
    ...
    public boolean initApp(AppProperties props) {
        ...
        // Database start
        try {
            //Aqui nos conectamos a la base de datos si se genera una exepcion el False recae en el iniciador
            //de este metodo osea JRootFrame o JRootKiosk que toma entonces la desicion de abrir JFrmConfig
            //para que el administrador arregle los errores de configuracion existentes....
            session = AppViewConnection.createSession(m_props);
        } catch (BasicException e) {
            JMessageDialog.showMessage(this, new MessageInf(MessageInf.SGN_DANGER, e.getMessage(), e));
            //Se muestra al usuario el error generado en la configuracion con el estandar establecido 
            return false;
        }
        ...
        //Necesitaria quitar esta parte para que no halla problema e compatibilidad con la BD si cambio el nombre y version app
    ...
    // Crea los botones del Login asociado a todo slos usuarios existentes en la BD
    // a su vez los asocia a una accion con AppUserAction  que ejecuta el login
    // con todos sus pasos ya que tiene el actionPerformed(event)
    ...
    // La accion del selector de los usuarios, cuando se pulsan lsoi JButtons de
    // en la seleccion del Login
    ...
    /*
     Este metodo openAppView es el que se llama para abrir la aplicacion luego
     del login con los privilegios de un usuario en especifico pasado como user
     del tipo AppUser, Inicia JPrincipalApp (muy importante)
     */
    ...
    private void initComponents() {
        ...
        jLabel1.setBackground(new java.awt.Color(253, 18, 64));
